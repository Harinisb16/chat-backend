...
function registerTools(server) {
    // --- Ticket tools ---
    server.registerTool("ticket.create", {
        title: "Create Ticket",
        description: "Create a new ticket",
        inputSchema: {
            title: zod_1.z.string(),
            description: zod_1.z.string(),
            ownedby: zod_1.z.string(),
            reportingmanager: zod_1.z.string(),
            priority: zod_1.z.enum(["low", "medium", "high"]).default("medium"),
        },
    }, async (args) => {
        const ticket = await ticket_service_1.TicketService.createTicket(args);
        return {
            content: [
    ...
    server.registerTool("ticket.updateStatus", {
        title: "Update Ticket Status",
        description: "Change the status of a ticket",
        inputSchema: { id: zod_1.z.number(), status: zod_1.z.string() },
    }, async ({ id, status }) => {
        const ticket = await ticket_service_1.TicketService.updateTicket(id, { status });
        return {
            content: [
    ...
    // --- User tools ---
    server.registerTool("user.create", {
        title: "Create User",
        description: "Create a new user",
        inputSchema: { username: zod_1.z.string(), email: zod_1.z.string() },
    }, async (args) => {
        return {
            content: [
                {
    ...
    server.registerTool("user.assignToTeam", {
        title: "Assign User to Team",
        description: "Assign an existing user to a team",
        inputSchema: { userId: zod_1.z.number(), teamId: zod_1.z.number() },
    }, async ({ userId, teamId }) => {
    ...
    server.registerTool("app.summary", {
        title: "Application Summary",
        description: "Return details about the ticketing application",
        inputSchema: {},
    }, async () => {
