...
exports.fetchMessagesBetweenUsers = exports.getAdmin = exports.fetchMessages = exports.fetchChatsForUser = void 0;
...
const fetchChatsForUser = async (req, res) => {
    try {
        ...
        const userIdNum = Number(userId);
        // 1️⃣ Get logged user
        const users = await db_1.sequelize.query(`SELECT "userId", "roleId"
       FROM "tbl_user"
       WHERE "userId" = :userId`, {
            replacements: { userId: userIdNum },
        ...
        if (loggedUser.roleId === 1) {
            chats = await db_1.sequelize.query(`SELECT "userId" as id, "username" as name
         FROM "tbl_user"
         WHERE "roleId" != 1`, { type: sequelize_1.QueryTypes.SELECT });
        }
        else {
...
exports.fetchChatsForUser = fetchChatsForUser;
...
const fetchMessages = async (req, res) => {
    try {
        ...
        // );
        // chat.controller.ts
        const messages = await db_1.sequelize.query(`
  SELECT * FROM "messages"
  WHERE ("senderId" = :userId AND "receiverId" = :adminId)
     OR ("senderId" = :adminId AND "receiverId" = :userId)
...
exports.fetchMessagesBetweenUsers = fetchMessagesBetweenUsers;
